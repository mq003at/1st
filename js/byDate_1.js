		...
		for(var i = month; i <= numOfMonths; i++, month++) {
			...
			for(var j = day; j <= numOfDays; j++) {
				...
				var dir2 = "0;";
					...
					x.setAttribute("id", id + "" + date_tr + "id");
					y.appendChild(x);
					...
					var x = document.createElement("TD");
					x.setAttribute('align', 'center');
					x.setAttribute("id", id + "" + date_tr + "dir1");
					y.appendChild(x);
					var t = document.createTextNode(dir1);
					x.appendChild(t);	
					...
					var x = document.createElement("TD");
					x.setAttribute('align', 'center');
					x.setAttribute("id", id + "" + date_tr + "dir2");
					y.appendChild(x);
					var t = document.createTextNode(dir2);
					x.appendChild(t);	
			
						
					x = document.createElement("TD");
					x.setAttribute('align', 'center');
					x.setAttribute("id", id + "" + date_tr + "des2");
					y.appendChild(x);
					x.innerHTML = '';
	...
	group_ref.once('value', snap => {
		...
		if(val != null) {
			Object.keys(val).forEach(function(key) {
				...
				$('#time_table').append('<tbody id="' + tag_id + '"></tbody>');
				$('#' + tag_id).append();
				tableSetUp(tag_id);
...
function set_reportDate(timestamps, id,mode) {	
	...
	var dir1 = "1";
	var dir2 = "0";
	var date_td = yeara + "-" + montha + "-" + daya;

	var x = 2;
	var j = a.substring(8,10);
	var y = parseFloat(j);
    var result = y + x;
	...
	if (mode == "1")
	{
		...
		if(date >= start_date && date <= end_date) {
			...
			if(times.length > 0)
				$('#' + id + date + "des").append();
			$('#' + id + date + "des").append(date_td + " " + a.substring(8,10) + ":" + a.substring(10,12));
		}
		else
		{
		...
		if (mode == "1")
	{
		...
		if(date >= start_date && date <= end_date) {
			...
			var times = $('#' + id + date + "des2").html();
			if(times.length >= 0)
				$('#' + id + date + "des2").append();
			$('#' + id + date + "des2").append(date_td + " " + b.substring(8,10) + ":" + b.substring(10,12) + ";" + result + ":" + a.substring(10,12) + " " + "0:30");	
		}
	}
	else
	{
		...
		else
		{
			...
			if(date >= start_date && date <= end_date) {
				...
				if(times.length > 0)
					$('#' + id + date + "des2").append();
				$('#' + id + date + "des2").append(date_td + " " + b.substring(8,10) + ":" + b.substring(10,12));
...
function fnExcelReport() {
	...
	//get table HTML code
	 tab_text = tab_text + $('#time_table').html();
